...
# Description: This script processes and aggregates traffic data into minute intervals,
...
def process_perminutedata(cur, start_time, end_time):
    logging.debug(f"Processing PerMinuteData from {start_time} to {end_time}")
    cache_key = get_cache_key("perminutedata", start_time, end_time)
    cached_data = mc.get(cache_key)
    
    if cached_data:
        logging.info(f"Retrieved PerMinuteData from cache")
        return json.loads(cached_data)
    
    per_second_data = calculate_per_second_data(cur, start_time, end_time)
    
    aggregated_data = []
    total_minutes = len(per_second_data) // 60
    
    logging.info(f"Total seconds of data: {len(per_second_data)}")
    logging.info(f"Expected number of minute intervals: {total_minutes}")
    
    with tqdm(total=total_minutes, desc="Aggregating minute data", unit="minute") as pbar:
        for i in range(0, len(per_second_data), 60):  # 60 seconds in a minute
            chunk = per_second_data[i:i+60]
            if not chunk:
                continue
    ...
    if aggregated_data:
        ...
        logging.info(f"Inserted/Updated {len(aggregated_data)} rows in PerMinuteData")
        
        mc.set(cache_key, json.dumps(aggregated_data), time=3600)  # Cache for 1 hour
    else:
        logging.warning(f"No data to aggregate for PerMinuteData")
    
    logging.info(f"Final number of PerMinuteData rows: {len(aggregated_data)}")
    ...
    try:
        with psycopg2.connect(**db_credentials) as conn:
            ...
            with conn.cursor() as cur:
                ...
                available_minutes = (max_timestamp - min_timestamp) / (1000 * 60)
                
                logging.info(f"Processing data from {min_datetime} to {max_datetime}")
                logging.info(f"Total available minutes: {available_minutes:.2f}")
                
                clear_table("PerMinuteData")
                create_perminutedata_table()
                process_perminutedata(cur, min_timestamp, max_timestamp)
                
                logging.info("PerMinuteData processed and committed successfully.")
