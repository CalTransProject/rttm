...
def process_per_minute_data(cur, start_time, end_time):
    cur.execute("""
        SELECT 
            "Timestamp",
            "TotalVehicles",
    ...
    for row in cur.fetchall():
        ...
        if minute not in per_minute_data:
            per_minute_data[minute] = {
                "TotalVehicles": 0,
                "TotalSpeed": 0,
                "TotalDensity": 0,
                "TotalConfidence": 0,
        ...
        per_minute_data[minute]["TotalVehicles"] += total_vehicles
        per_minute_data[minute]["TotalSpeed"] += average_speed
        per_minute_data[minute]["TotalDensity"] += density
        per_minute_data[minute]["TotalConfidence"] += average_confidence
        per_minute_data[minute]["Count"] += 1

        for vehicle_type, count in vehicle_type_counts.items():
            per_minute_data[minute]["VehicleTypeCounts"][vehicle_type] = per_minute_data[minute]["VehicleTypeCounts"].get(vehicle_type, 0) + count

        for lane, count in lane_vehicle_counts.items():
            per_minute_data[minute]["LaneVehicleCounts"][lane] = per_minute_data[minute]["LaneVehicleCounts"].get(lane, 0) + count

        for lane, type_counts in lane_type_counts.items():
            if lane not in per_minute_data[minute]["LaneTypeCounts"]:
                per_minute_data[minute]["LaneTypeCounts"][lane] = {}
            for vehicle_type, count in type_counts.items():
                per_minute_data[minute]["LaneTypeCounts"][lane][vehicle_type] = per_minute_data[minute]["LaneTypeCounts"][lane].get(vehicle_type, 0) + count

    processed_per_minute_data = []
    for minute, data in per_minute_data.items():
        processed_per_minute_data.append((
            minute * 60,
            data["TotalVehicles"],
    ...
    if processed_per_minute_data:
        ...
        placeholders = ', '.join(['%s'] * len(processed_per_minute_data[0]))

        query = f"INSERT INTO \"PerMinuteData\" ({columns}) VALUES ({placeholders}) ON CONFLICT (\"Timestamp\") DO UPDATE SET \"TotalVehicles\" = EXCLUDED.\"TotalVehicles\", \"AverageSpeed\" = EXCLUDED.\"AverageSpeed\", \"Density\" = EXCLUDED.\"Density\", \"AverageConfidence\" = EXCLUDED.\"AverageConfidence\", \"VehicleTypeCounts\" = EXCLUDED.\"VehicleTypeCounts\", \"LaneVehicleCounts\" = EXCLUDED.\"LaneVehicleCounts\", \"LaneTypeCounts\" = EXCLUDED.\"LaneTypeCounts\";"

        try:
            cur.executemany(query, processed_per_minute_data)
        except Exception as e:
            print(f"Error inserting data: {e}")

def create_per_minute_table(cur):
    cur.execute("""
...
with psycopg2.connect(**db_credentials) as conn:
    with conn.cursor() as cur:
        ...
        create_per_minute_table(cur)

        cur.execute("SELECT MIN(\"Timestamp\"), MAX(\"Timestamp\") FROM \"PerSecondData\";")
        result = cur.fetchone()
        if result:
            start_timestamp, end_timestamp = result
            process_per_minute_data(cur, start_timestamp, end_timestamp)
            print("Per-minute data processed and inserted into PerMinuteData successfully.")
        else:
