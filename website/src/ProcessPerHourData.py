...
def process_per_hour_data(cur, start_time, end_time):
    ...
    for row in cur.fetchall():
        ...
        per_hour_data[hour]["TotalVehicles"] += total_vehicles
        per_hour_data[hour]["TotalSpeed"] += average_speed
        per_hour_data[hour]["TotalDensity"] += density
        per_hour_data[hour]["TotalConfidence"] += average_confidence
        per_hour_data[hour]["Count"] += 1

        for vehicle_type, count in vehicle_type_counts.items():
            per_hour_data[hour]["VehicleTypeCounts"][vehicle_type] = per_hour_data[hour]["VehicleTypeCounts"].get(vehicle_type, 0) + count

        for lane, count in lane_vehicle_counts.items():
            per_hour_data[hour]["LaneVehicleCounts"][lane] = per_hour_data[hour]["LaneVehicleCounts"].get(lane, 0) + count

        for lane, type_counts in lane_type_counts.items():
            if lane not in per_hour_data[hour]["LaneTypeCounts"]:
                per_hour_data[hour]["LaneTypeCounts"][lane] = {}
            for vehicle_type, count in type_counts.items():
                per_hour_data[hour]["LaneTypeCounts"][lane][vehicle_type] = per_hour_data[hour]["LaneTypeCounts"][lane].get(vehicle_type, 0) + count

    processed_per_hour_data = []
    for hour, data in per_hour_data.items():
        processed_per_hour_data.append((
            hour * 3600,
            data["TotalVehicles"],
    ...
    if processed_per_hour_data:
        ...
        placeholders = ', '.join(['%s'] * len(processed_per_hour_data[0]))

        query = f"INSERT INTO \"PerHourData\" ({columns}) VALUES ({placeholders}) ON CONFLICT (\"Timestamp\") DO UPDATE SET \"TotalVehicles\" = EXCLUDED.\"TotalVehicles\", \"AverageSpeed\" = EXCLUDED.\"AverageSpeed\", \"Density\" = EXCLUDED.\"Density\", \"AverageConfidence\" = EXCLUDED.\"AverageConfidence\", \"VehicleTypeCounts\" = EXCLUDED.\"VehicleTypeCounts\", \"LaneVehicleCounts\" = EXCLUDED.\"LaneVehicleCounts\", \"LaneTypeCounts\" = EXCLUDED.\"LaneTypeCounts\";"

        try:
            cur.executemany(query, processed_per_hour_data)
...
with psycopg2.connect(**db_credentials) as conn:
    with conn.cursor() as cur:
        ...
        if result:
            ...
            process_per_hour_data(cur, start_timestamp, end_timestamp)
            print("Per-hour data processed and inserted into PerHourData successfully.")
        else:
